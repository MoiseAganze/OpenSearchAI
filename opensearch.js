...
async function scrapePageContent(url, useProxy) {
    try {
        ...
        let siteName = $("meta[property='og:site_name']").attr("content") || url.split("/")[2];
        let favicon = $("link[rel='icon']").attr("href") || `https://${url.split("/")[2]}/favicon.ico`;

        return { title, content, contentSize, siteName, favicon, url };
    } catch (error) {
        console.error(`Erreur lors du scraping de la page ${url}:`, error.message);
        return { title: "", content: "", contentSize: 0, siteName: "", favicon: "", url };
    }
}
